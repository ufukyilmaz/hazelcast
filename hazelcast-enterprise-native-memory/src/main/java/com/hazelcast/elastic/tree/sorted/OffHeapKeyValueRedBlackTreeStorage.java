...
import com.hazelcast.elastic.tree.iterator.OffHeapKeyRedBlackTreeKeysIteratorImpl;
import com.hazelcast.elastic.tree.iterator.value.OffHeapValueIterator;
import com.hazelcast.elastic.tree.iterator.value.OffHeapValueIteratorImpl;
import com.hazelcast.internal.memory.GlobalMemoryAccessor;
import com.hazelcast.internal.memory.GlobalMemoryAccessorRegistry;
import com.hazelcast.internal.memory.MemoryAllocator;
import com.hazelcast.nio.Bits;

import static com.hazelcast.elastic.tree.sorted.OrderingDirection.ASC;
import static com.hazelcast.elastic.tree.sorted.OrderingDirection.DESC;
...
public class OffHeapKeyValueRedBlackTreeStorage implements OffHeapKeyValueSortedStorage {
    ...
    protected static final GlobalMemoryAccessor UNSAFE = GlobalMemoryAccessorRegistry.MEM;
    ...
    @Override
    public OffHeapKeyIterator keyIterator(long keyEntryAddress) {
        return keyIterator(ASC, keyEntryAddress);
    }
    ...
    public OffHeapKeyIterator keyIterator(OrderingDirection direction) {
        OffHeapKeyRedBlackTreeKeysIteratorImpl it = new OffHeapKeyRedBlackTreeKeysIteratorImpl(this);
     ...
     */
    @Override
    public OffHeapKeyIterator keyIterator(OrderingDirection direction, long keyEntryAddress) {
        OffHeapKeyRedBlackTreeKeysIteratorImpl it = new OffHeapKeyRedBlackTreeKeysIteratorImpl(this);
        it.setDirection(direction, keyEntryAddress);
        return it;
    }
    ...
    public OffHeapValueIterator valueIterator(long keyEntryPointer) {
        OffHeapValueIteratorImpl it = new OffHeapValueIteratorImpl(this);
        it.reset(keyEntryPointer);
        return it;
    }
    ...
    @Override
    public long searchKeyEntry(long keyAddress, long keyWrittenBytes, long keyAllocatedBytes) {
        if (this.rootAddress == 0L) {
            return 0L;
        }
        ...
        while (true) {
            int compareResult = compareKeys(this.offHeapKeyComparator, keyAddress, keyWrittenBytes, address);
    ...
    protected long getKeyEntry0(long rootAddress,
        ...
        while (true) {
            ...
            if (compareResult > 0) { //Our key is greater
                long rightAddress = getRightAddressOffset(address);
            ...
            } else if (compareResult < 0) { //Our key is less
                long leftAddress = getLeftAddressOffset(address);
    ...
    @Override
    public void remove(long keyAddress) {
        // TODO implement RBT element removal
    }

...
abstract class AbstractOffHeapCacheOperation
    ...
    transient OffHeapCacheRecordStore cache;
    ...
    public final void beforeRun() throws Exception {
        ...
        try {
            ...
            if (this instanceof BackupAwareOffHeapCacheOperation) {
                cache = (OffHeapCacheRecordStore) service.getOrCreateCache(name, getPartitionId());
            } else {
                cache = (OffHeapCacheRecordStore) service.getCacheRecordStore(name, getPartitionId());
            }
        } catch (Throwable e) {
            dispose();

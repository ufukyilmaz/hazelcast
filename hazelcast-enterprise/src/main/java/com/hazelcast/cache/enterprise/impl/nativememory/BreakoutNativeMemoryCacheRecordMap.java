...
import com.hazelcast.cache.enterprise.BreakoutCacheRecordMap;
 ...
 */
public final class BreakoutNativeMemoryCacheRecordMap
        extends BinaryElasticHashMap<BreakoutNativeMemoryCacheRecord>
        implements BreakoutCacheRecordMap<Data, BreakoutNativeMemoryCacheRecord> {
    ...
    private final transient BreakoutNativeMemoryCacheRecordAccessor cacheRecordAccessor;
    private final transient Callback<Data> evictionCallback;
    private int randomEvictionLastIndex;
    private final Random random = new Random();

    public BreakoutNativeMemoryCacheRecordMap(int initialCapacity, EnterpriseSerializationService serializationService,
            BreakoutNativeMemoryCacheRecordAccessor cacheRecordAccessor, Callback<Data> evictionCallback) {
        ...
        if (len > 0 && size() > 0) {
            ...
            for (int ix = start; ix < end; ix++) {
                if (isAllocated(ix)) {
                    ...
                    int ttlMillis = BreakoutNativeMemoryCacheRecord.getTtlMillis(value);
                    if (ttlMillis > 0) {
                        long creationTime = BreakoutNativeMemoryCacheRecord.getCreationTime(value);
    ...
    private long getAccessTime(long recordAddress) {
        long creationTime = BreakoutNativeMemoryCacheRecord.getCreationTime(recordAddress);
        int accessTimeDiff = BreakoutNativeMemoryCacheRecord.getAccessTimeDiff(recordAddress);
    ...
    private int evictRecordsLFU(int percentage) {
        ...
        if (size > 0) {
            ...
            for (int ix = 0; ix < capacity; ix++) {
                if (isAllocated(ix)) {
                    long value = getValue(ix);
                    hit = BreakoutNativeMemoryCacheRecord.getAccessHit(value);
                    sortArray[k] = hit;
                    if (++k >= size) {
                        break;
                    }
            ...
            for (int ix = 0; ix < capacity && k < index; ix++) {
                if (isAllocated(ix)) {
                    long value = getValue(ix);
                    int h = BreakoutNativeMemoryCacheRecord.getAccessHit(value);
                    if (h <= hit) {
                        k++;
                        long key = getKey(ix);
                        NativeMemoryData keyData = cacheRecordAccessor.readData(key);
    ...
    public CacheKeyIteratorResult fetchNext(int nextTableIndex, int size) {
        long now = Clock.currentTimeMillis();
        BinaryElasticHashMap<BreakoutNativeMemoryCacheRecord>.EntryIter iter =
                iterator(nextTableIndex);
        List<Data> keys = new ArrayList<Data>();
        for (int i = 0; i < size && iter.hasNext(); i++) {
            Map.Entry<Data, BreakoutNativeMemoryCacheRecord> entry = iter.next();
            Data key = entry.getKey();
            BreakoutNativeMemoryCacheRecord record = entry.getValue();

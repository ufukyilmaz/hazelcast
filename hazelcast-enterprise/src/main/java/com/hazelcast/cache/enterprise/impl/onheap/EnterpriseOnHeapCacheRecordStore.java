 ...
 */
public class EnterpriseOnHeapCacheRecordStore extends AbstractEnterpriseCacheRecordStore {
    ...
    protected final EnterpriseOnHeapCacheHashMap<Data, EnterpriseOnHeapCacheRecord> records =
            new EnterpriseOnHeapCacheHashMap<Data, EnterpriseOnHeapCacheRecord>(1000);
    protected final EnterpriseOnHeapCacheRecordFactory cacheRecordFactory;
    ...
    public EnterpriseOnHeapCacheRecordStore(final String name,
        ...
        this.cacheRecordFactory = new EnterpriseOnHeapCacheRecordFactory(cacheConfig.getInMemoryFormat(), nodeEngine.getSerializationService());
    ...
    public Object get(Data key, ExpiryPolicy expiryPolicy) {
        ...
        Object value;
        EnterpriseOnHeapCacheRecord record = records.get(key);
    ...
    private long updateAccessDuration(EnterpriseOnHeapCacheRecord record, ExpiryPolicy localExpiryPolicy, long now) {
        long expiryTime = -1L;
        try {
    ...
    public void removeAll(Set<Data> keys) {
        ...
        try {
            ...
            for (Data key : keysToClean) {
                ...
                final EnterpriseOnHeapCacheRecord record = records.get(key);
    ...
    public EnterpriseOnHeapCacheRecord getRecord(Data key) {
        return records.get(key);
    }

    @Override
    public void setRecord(Data key, CacheRecord record) {
        if (!(record instanceof EnterpriseOnHeapCacheRecord)) {
            throw new IllegalArgumentException("record must be an instance of "
                    + EnterpriseOnHeapCacheRecord.class.getName() + "!");
        }
        if (key != null && record != null) {
            records.put(key, (EnterpriseOnHeapCacheRecord)record);
        }
    }

    @Override
    public EnterpriseOnHeapCacheRecord removeRecord(Data key) {
        if (key != null) {
    ...
    boolean createRecordWithExpiry(Data key, Object value, ExpiryPolicy localExpiryPolicy, long now,
        ...
        if (!isExpiredAt(expiryTime, now)) {
            EnterpriseOnHeapCacheRecord record = createRecord(key, value, expiryTime);
    ...
    private EnterpriseOnHeapCacheRecord createRecord(Data keyData, Object value, long expirationTime) {
    ...
    boolean updateRecordWithExpiry(Data key, Object value, EnterpriseOnHeapCacheRecord record, ExpiryPolicy localExpiryPolicy, long now,
                                   boolean disableWriteThrough) {
        long expiryTime = -1L;
        try {
    ...
    private EnterpriseOnHeapCacheRecord updateRecord(EnterpriseOnHeapCacheRecord record, Object value) {
    ...
    void deleteRecord(Data key) {
        final EnterpriseOnHeapCacheRecord record = records.remove(key);
        final Data dataValue;
        switch (cacheConfig.getInMemoryFormat()) {
            case BINARY:
    ...
    EnterpriseOnHeapCacheRecord accessRecord(EnterpriseOnHeapCacheRecord record, ExpiryPolicy expiryPolicy, long now) {
        final ExpiryPolicy localExpiryPolicy = expiryPolicy != null ? expiryPolicy : defaultExpiryPolicy;
        updateAccessDuration(record, localExpiryPolicy, now);
        return record;
    }

    EnterpriseOnHeapCacheRecord readThroughRecord(Data key, long now) {
        final ExpiryPolicy localExpiryPolicy = defaultExpiryPolicy;
        ...
        final EnterpriseOnHeapCacheRecord record = createRecord(key, value, expiryTime);
        return record;
    }
    ...
    protected boolean processExpiredEntry(Data key, EnterpriseOnHeapCacheRecord record, long now) {
    ...
    protected boolean processExpiredEntry(Data key, EnterpriseOnHeapCacheRecord record, long expiryTime, long now) {

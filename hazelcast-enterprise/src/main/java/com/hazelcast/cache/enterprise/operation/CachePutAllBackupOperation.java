...
public class CachePutAllBackupOperation
    ...
    private transient BreakoutCacheRecordStore cache;
    ...
    protected void writeInternal(ObjectDataOutput out) throws IOException {
        ...
        if (cacheRecords != null) {
            ...
            for (Map.Entry<Data, CacheRecord> entry : cacheRecords.entrySet()) {
                ...
                if (record instanceof BreakoutCacheRecord) {
                    out.writeBoolean(true);
                    writeHiDensityCacheRecord(out, serializationService,
                            (BreakoutCacheRecord) record);
                } else {
    ...
    private void writeHiDensityCacheRecord(ObjectDataOutput out,
                                           ...
                                           BreakoutCacheRecord record) throws IOException {
        if (record == null) {
            out.writeBoolean(false);
            return;
        }
    ...
    private BreakoutCacheRecord readHiDensityCacheRecord(
        ...
        try {
            ...
            if (nativeMemoryData != null && nativeMemoryData.address() != MemoryManager.NULL_ADDRESS) {
                cache.getCacheRecordAccessor().disposeData(nativeMemoryData);
            }

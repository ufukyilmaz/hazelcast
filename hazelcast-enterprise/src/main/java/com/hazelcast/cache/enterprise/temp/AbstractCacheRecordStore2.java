...
public abstract class AbstractCacheRecordStore2 implements ICacheRecordStore {
    ...
    protected final EnterpriseCacheHashMap2 records;
    ...
    protected AbstractCacheRecordStore2(final CacheConfig cacheConfig,
        ...
        this.records = new EnterpriseCacheHashMap2(initialCapacity, serializationService, cacheRecordService, createEvictionCallback());
    ...
    public AbstractCacheRecordStore2(final String cacheName,
                                     final EnterpriseCacheService cacheService,
    ...
    public AbstractCacheRecordStore2(final CacheConfig cacheConfig,
                                     final EnterpriseCacheService cacheService,
                                     final EnterpriseSerializationService ss,
                                     final NodeEngine nodeEngine,
                                     final int initialCapacity) {
    ...
    protected OffHeapData getRecordData(EnterpriseCacheRecord2 record) {
    ...
    public boolean createRecordWithExpiry(Data key,
        ...
        if (!isExpiredAt(expiryTime, now)) {
            EnterpriseCacheRecord2 record = createRecord(key, value, expiryTime);
    ...
    public EnterpriseCacheRecord2 createRecord(Data keyData,
                                              Object value,
                                              long expirationTime) {
        final EnterpriseCacheRecord2 record =
    ...
    public EnterpriseCacheRecord2 updateRecord(EnterpriseCacheRecord2 record, Object value) {
    ...
    public void deleteRecord(Data key) {
        final EnterpriseCacheRecord2 record = records.remove(key);
    ...
    public EnterpriseCacheRecord2 accessRecord(EnterpriseCacheRecord2 record,
    ...
    public EnterpriseCacheRecord2 readThroughRecord(Data key, long now) {
    ...
    protected boolean processExpiredEntry(Data key, EnterpriseCacheRecord2 record, long now) {
    ...
    protected boolean processExpiredEntry(Data key, EnterpriseCacheRecord2 record, long expiryTime, long now) {
    ...
    protected long updateAccessDuration(EnterpriseCacheRecord2 record,
                                        ExpiryPolicy expiryPolicy,
                                        long now) {
        long expiryTime = -1L;
        try {
    ...
    protected void onAccess(long now, EnterpriseCacheRecord2 record, long creationTime) {
        if (evictionEnabled) {
    ...
    protected EnterpriseCacheRecord2 createRecord(long now) {
        return createRecord(null, now, -1);
    }

    protected EnterpriseCacheRecord2 createRecord(Object value, long now) {
        return createRecord(value, now, -1);
    }

    protected EnterpriseCacheRecord2 createRecord(long now, long expirationTime) {
        return createRecord(null, now, -1);
    }

    protected EnterpriseCacheRecord2 createRecord(Object value, long now, long expirationTime) {
        long address = memoryManager.allocate(EnterpriseCacheRecord2.SIZE);
        EnterpriseCacheRecord2 record = cacheRecordService.newRecord();
        ...
        if (value != null) {
            ...
            record.setValueAddress(EnterpriseOffHeapCacheRecordStore.NULL_PTR);
        }
        return record;
    }
    ...
    protected Data putInternal(Data key,
        ...
        long creationTime;
        EnterpriseCacheRecord2 record = null;
        ...
        try {
            ...
            if (record == null) {
                ...
                if (record.getValueAddress() == EnterpriseOffHeapCacheRecordStore.NULL_PTR) {
            ...
            if (newPut && record != null && record.address() != EnterpriseOffHeapCacheRecordStore.NULL_PTR) {
                if (!records.delete(key)) {
                    cacheRecordService.dispose(record);
                }
            }
            if (newValue != null && newValue.address() != EnterpriseOffHeapCacheRecordStore.NULL_PTR) {
    ...
    public EnterpriseCacheHashMap2 getCacheMap() {
        return records;
    }
    ...
    public Data get(Data key, ExpiryPolicy expiryPolicy) {
        ...
        EnterpriseCacheRecord2 record = records.get(key);
    ...
    public void setRecord(Data key, CacheRecord record) {
        if (!(record instanceof EnterpriseCacheRecord2)) {
            throw new IllegalArgumentException("record must be an instance of "
                    + EnterpriseCacheRecord2.class.getName());
        }
        records.set(key, (EnterpriseCacheRecord2)record);
    }
    ...
    public boolean putIfAbsent(Data key, Object value, ExpiryPolicy expiryPolicy, String caller) {
        ...
        try {
            ...
            if (record != null && record.address() != EnterpriseOffHeapCacheRecordStore.NULL_PTR) {
                if (!records.delete(key)) {
                    cacheRecordService.dispose(record);
                }
            }
    ...
    public Data getAndRemove(Data key, String caller) {
        Data oldValue = null;
        EnterpriseCacheRecord2 record = records.remove(key);
        if (record != null) {
            onEntryInvalidated(key, caller);
            OffHeapData oldBinary = cacheRecordService.readData(record.getValueAddress());
            oldValue = serializationService.convertData(oldBinary, DataType.HEAP);
    ...
    public void removeAll(Set<Data> keys) {
        ...
        try {
            ...
            for (Data key : keysToClean) {
                ...
                final EnterpriseCacheRecord2 record = records.get(key);
    ...
    public static class CacheRecordAccessor
            implements MemoryBlockAccessor<EnterpriseCacheRecord2> {

        private final EnterpriseSerializationService ss;
        private final MemoryManager memoryManager;
        private final Queue<EnterpriseCacheRecord2> recordQ = new ArrayDeque<EnterpriseCacheRecord2>(1024);
        ...
        @Override
        public boolean isEqual(long address, EnterpriseCacheRecord2 value) {
            return isEqual(address, value.address());
        }

        @Override
        public boolean isEqual(long address1, long address2) {
            long valueAddress1 = UnsafeHelper.UNSAFE.getLong(address1 + EnterpriseCacheRecord2.VALUE_OFFSET);
            long valueAddress2 = UnsafeHelper.UNSAFE.getLong(address2 + EnterpriseCacheRecord2.VALUE_OFFSET);
            return OffHeapDataUtil.equals(valueAddress1, valueAddress2);
        }

        public EnterpriseCacheRecord2 newRecord() {
            EnterpriseCacheRecord2 record = recordQ.poll();
            if (record == null) {
                record = new EnterpriseCacheRecord2();
            }
            return record;
        }

        @Override
        public EnterpriseCacheRecord2 read(long address) {
            if (address <= EnterpriseOffHeapCacheRecordStore.NULL_PTR) {
                throw new IllegalArgumentException("Illegal memory address: " + address);
            }
            EnterpriseCacheRecord2 record = newRecord();
            record.reset(address);
            return record;
        }

        @Override
        public void dispose(EnterpriseCacheRecord2 record) {
            if (record.address() <= EnterpriseOffHeapCacheRecordStore.NULL_PTR) {
                throw new IllegalArgumentException("Illegal memory address: " + record.address());
            }
            disposeValue(record);
            record.clear();
            memoryManager.free(record.address(), record.size());
            recordQ.offer(record.reset(EnterpriseOffHeapCacheRecordStore.NULL_PTR));
        }
        ...
        public OffHeapData readData(long valueAddress) {
            if (valueAddress <= EnterpriseOffHeapCacheRecordStore.NULL_PTR) {
        ...
        public void disposeValue(EnterpriseCacheRecord2 record) {
            long valueAddress = record.getValueAddress();
            if (valueAddress != EnterpriseOffHeapCacheRecordStore.NULL_PTR) {
        ...
        void enqueueRecord(EnterpriseCacheRecord2 record) {
            recordQ.offer(record.reset(EnterpriseOffHeapCacheRecordStore.NULL_PTR));
        }

        void enqueueData(OffHeapData data) {
            data.reset(EnterpriseOffHeapCacheRecordStore.NULL_PTR);
            dataQ.offer(data);
        }
    }

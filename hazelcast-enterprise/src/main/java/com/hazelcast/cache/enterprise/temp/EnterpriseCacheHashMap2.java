...
public final class EnterpriseCacheHashMap2 extends BinaryOffHeapHashMap<EnterpriseCacheRecord2> {
    ...
    public EnterpriseCacheHashMap2(int initialCapacity, EnterpriseSerializationService serializationService,
                                   MemoryBlockAccessor<EnterpriseCacheRecord2> memoryBlockAccessor, Callback<Data> evictionCallback) {
    ...
    public int evictExpiredRecords(int percentage) {
        ...
        int k = 0;
        if (len > 0 && size() > 0) {
            AbstractCacheRecordStore2.CacheRecordAccessor service = getCacheRecordAccessor();
            ...
            for (int ix = start; ix < end; ix++) {
                if (isAllocated(ix)) {
                    ...
                    int ttlMillis = EnterpriseCacheRecord2.getTtlMillis(value);
                    if (ttlMillis > 0) {
                        long creationTime = EnterpriseCacheRecord2.getCreationTime(value);
    ...
    private static long getAccessTime(long recordAddress) {
        long creationTime = EnterpriseCacheRecord2.getCreationTime(recordAddress);
        int accessTimeDiff = EnterpriseCacheRecord2.getAccessTimeDiff(recordAddress);
    ...
    private int evictRecordsLFU(int percentage) {
        ...
        if (size > 0) {
            ...
            for (int ix = 0; ix < capacity; ix++) {
                if (isAllocated(ix)) {
                    long value = getValue(ix);
                    hit = EnterpriseCacheRecord2.getAccessHit(value);
                    sortArray[k] = hit;
                    if (++k >= size) {
                        break;
                    }
            ...
            for (int ix = 0; ix < capacity && k < index; ix++) {
                if (isAllocated(ix)) {
                    long value = getValue(ix);
                    int h = EnterpriseCacheRecord2.getAccessHit(value);
                    if (h <= hit) {
                        k++;
                        long key = getKey(ix);
                        OffHeapData keyData = service.readData(key);
    ...
    private AbstractCacheRecordStore2.CacheRecordAccessor getCacheRecordAccessor() {
        return (AbstractCacheRecordStore2.CacheRecordAccessor) memoryBlockAccessor;
    }

...
public class BinaryOffHeapHashMap<V extends MemoryBlock> implements OffHeapMap<Data, V> {

    private static final long allocationFactor = 17L;

    protected final EnterpriseSerializationService serializationService;

    protected final MemoryBlockAccessor<V> memoryBlockAccessor;
     ...
     */
    public BinaryOffHeapHashMap(EnterpriseSerializationService serializationService,
            MemoryBlockAccessor<V> memoryBlockAccessor,
     ...
     * @param malloc
     */
    public BinaryOffHeapHashMap(int initialCapacity, EnterpriseSerializationService serializationService,
     ...
     * @param malloc
     */
    public BinaryOffHeapHashMap(int initialCapacity, float loadFactor, EnterpriseSerializationService serializationService,
                                MemoryBlockAccessor<V> memoryBlockAccessor, MemoryAllocator malloc) {
    ...
    public V put(Data key, MemoryBlock value) {
        ...
        while (isAllocated(slot)) {
            ...
            if (OffHeapDataUtil.equals(slotKey, key)) {
                final long oldValue = getValue(slot);
                setValue(slot, value.address());
                if (key instanceof OffHeapData && ((OffHeapData) key).address() != slotKey) {
                    serializationService.disposeData(key);
                }
                return memoryBlockAccessor.read(oldValue);
            }
            slot = (slot + 1) & mask;
        }

        OffHeapData memKey = serializationService.convertData(key, DataType.OFFHEAP);
    ...
    public boolean set(Data key, V value) {
        V old = put(key, value);
        if (old != null) {
            memoryBlockAccessor.dispose(old);
        }
        return old == null;
    }
    ...
    public boolean replace(Data key, V oldValue, V newValue) {
        ensureMemory();
        assert newValue instanceof OffHeapData;
        ...
        while (isAllocated(slot)) {
            ...
            if (OffHeapDataUtil.equals(slotKey, key)) {
                ...
                if (memoryBlockAccessor.isEqual(current, oldValue)) {
                    setValue(slot, newValue.address());
                    memoryBlockAccessor.dispose(current);
                    return true;
                }
                return false;
            }
    ...
    public V replace(Data key, MemoryBlock value) {
        ...
        while (isAllocated(slot)) {
            ...
            if (OffHeapDataUtil.equals(slotKey, key)) {
                long current = getValue(slot);
                setValue(slot, value.address());
                return memoryBlockAccessor.read(current);
            }
            slot = (slot + 1) & mask;
            if (slot == wrappedAround) break;
        }
    ...
    private OffHeapData readData(long address) {
        if (address > 0L) {
    ...
    private void expandAndPut(long pendingKey, long pendingValue, int freeSlot) {
        ...
        for (int i = oldAllocatedLength; --i >= 0; ) {
            if (readBool(oldAllocated, i)) {
                ...
                int slot = rehash(OffHeapDataUtil.hashCode(k), perturbation) & mask;
                while (isAllocated(slot)) {
                    slot = (slot + 1) & mask;
                }
        ...
        while (isAllocated(slot)) {
            ...
            if (OffHeapDataUtil.equals(slotKey, key)) {
                ...
                if (key instanceof HeapData ||
                        (key instanceof OffHeapData && ((OffHeapData) key).address() != slotKey)) {
                    serializationService.disposeData(readData(slotKey));
                }
                shiftConflictingKeys(slot);
                return memoryBlockAccessor.read(v);
            }
            slot = (slot + 1) & mask;
            if (slot == wrappedAround) break;
        }
    ...
    public boolean delete(Data key) {
        V value = remove(key);
        if (value != null) {
            memoryBlockAccessor.dispose(value);
        }
        return value != null;
    }
    ...
    protected void shiftConflictingKeys(int slotCurr) {
        ...
        while (true) {
            ...
            while (isAllocated(slotCurr)) {
                slotOther = rehash(OffHeapDataUtil.hashCode(getKey(slotCurr)), perturbation) & mask;
    ...
    public V get(Object k) {
        ...
        while (isAllocated(slot)) {
            ...
            if (OffHeapDataUtil.equals(slotKey, key)) {
                ...
                return memoryBlockAccessor.read(value);
            }
            slot = (slot + 1) & mask;
            if (slot == wrappedAround) break;
        }
    ...
    private class KeySet extends AbstractSet<Data> {
        ...
        public int size() {
            return BinaryOffHeapHashMap.this.size();
        }
        public boolean contains(Object o) {
            return containsKey(o);
        }
        public boolean remove(Object o) {
            return BinaryOffHeapHashMap.this.remove(o) != null;
        }
        public void clear() {
            BinaryOffHeapHashMap.this.clear();
        }
    }
    ...
    public class ValueIter extends SlotIter<V> implements Iterator<V> {
        ...
        public V next() {
            ...
            return memoryBlockAccessor.read(slotValue);
        }
    }
...
//            }
    ...
    private class MapEntry implements Map.Entry<Data, V> {
        ...
        private MapEntry(final int slot) {this.slot = slot;}

        @Override
        public Data getKey() {
            return readData(BinaryOffHeapHashMap.this.getKey(slot));
        }
        @Override
        public V getValue() {
            return memoryBlockAccessor.read(BinaryOffHeapHashMap.this.getValue(slot));
        }
        @Override
        public V setValue(MemoryBlock value) {
            V current = getValue();
            BinaryOffHeapHashMap.this.setValue(slot, value.address());
            return current;
        }
    }
    ...
    public void clear() {
        if (baseAddress > 0L) {
            ...
            while (iter.hasNext()) {
                ...
                serializationService.disposeData(e.getKey());
                memoryBlockAccessor.dispose(e.getValue());
            }

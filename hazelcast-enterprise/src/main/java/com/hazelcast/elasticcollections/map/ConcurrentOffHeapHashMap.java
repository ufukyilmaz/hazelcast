...
*/
public class ConcurrentOffHeapHashMap<K, V> implements ConcurrentOffHeapMap<K, V> {
    ...
    public ConcurrentOffHeapHashMap(EnterpriseSerializationService ss, MemoryAllocator malloc) {
        this(DEFAULT_CAPACITY, DEFAULT_LOAD_FACTOR, DEFAULT_CONCURRENCY_LEVEL, ss, malloc);
    }

    public ConcurrentOffHeapHashMap(int initialCapacity, float loadFactor, int concurrencyLevel,
    ...
    protected static final class Segment<V> {
        ...
        final BinaryOffHeapHashMap<OffHeapData> map;
...
//        transient volatile int count;
        ...
        public V putIfAbsent(final Data key, V value) {
            ...
            try {
                lock.lock();
                try {
                    OffHeapData current = map.putIfAbsent(key, v);
                    if (current != null) {
                        ss.disposeData(key);
                        ss.disposeData(v);
        ...
        public boolean replace(final Data key, final V oldValue, final V newValue) {
            ...
            try {
                o = ss.toData(oldValue, DataType.OFFHEAP);
                n = ss.toData(newValue, DataType.OFFHEAP);
    ...
    public V get(Object key) {
        Data k = ss.toData(key, DataType.OFFHEAP);
        Segment<V> segment = segmentFor(k);
        try {

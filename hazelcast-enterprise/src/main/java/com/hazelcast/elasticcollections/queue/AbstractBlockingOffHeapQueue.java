 ...
 */
abstract class AbstractBlockingOffHeapQueue<E> extends AbstractQueue<E> implements BlockingOffHeapQueue<E> {

    private final LongBlockingQueue queue;
    private final EnterpriseSerializationService ss;

    protected AbstractBlockingOffHeapQueue(EnterpriseSerializationService ss, int capacity, MemoryAllocator malloc) {
        this.ss = ss;
    ...
    private E toItem(long address, boolean dispose) {
        if (address != queue.nullItem()) {
            OffHeapData data = new OffHeapData().reset(address);
            try {
    ...
    private class ConsumerImpl implements LongConsumer {
        ...
        public boolean consume(final long value) {
            if (count > 0) {
                OffHeapData data = new OffHeapData().reset(value);

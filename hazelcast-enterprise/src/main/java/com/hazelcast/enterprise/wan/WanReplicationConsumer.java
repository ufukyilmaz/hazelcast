...
/**
 * Interface to be implemented by custom WAN event consumers. Wan replication consumers are typically used in
 * conjunction with a custom {@link WanReplicationPublisher}. The publisher will then publish events in a custom fashion
...
public interface WanReplicationConsumer {
    ...
    /**
     * Initialize the WAN consumer. The method is invoked once the node has started.
     * The WAN consumer is responsible for registering itself for WAN event consumption. Typically this means that you
     * would either use the {@link ExecutionService} to schedule the consumer to run periodically or continually by
     * having an implementation which uses blocking or spinning locks to check for new events. The implementation is
     * free however to choose a third approach.
     *

...
public abstract class AbstractHiDensityRecordAccessor<R extends HiDensityRecord>
    ...
    public R read(long address) {
        if (address == JvmMemoryManager.NULL_ADDRESS) {
    ...
    public long dispose(R record) {
        if (record.address() == JvmMemoryManager.NULL_ADDRESS) {
        ...
        record.reset(JvmMemoryManager.NULL_ADDRESS);
        return size;
    }
    ...
    public NativeMemoryData readData(long valueAddress) {
        if (valueAddress == JvmMemoryManager.NULL_ADDRESS) {
    ...
    public long disposeData(NativeMemoryData value) {
        if (value.address() == JvmMemoryManager.NULL_ADDRESS) {
    ...
    public long getSize(MemoryBlock memoryBlock) {
        if (memoryBlock == null || memoryBlock.address() == JvmMemoryManager.NULL_ADDRESS) {
            return  0;
        }

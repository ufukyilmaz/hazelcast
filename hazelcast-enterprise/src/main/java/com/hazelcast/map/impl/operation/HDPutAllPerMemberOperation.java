...
/**
 * Inserts the {@link MapEntries} for all partitions of a member via locally invoked {@link HDPutAllOperation}.
 * <p/>
 * Used to reduce the number of remote invocations of an {@link com.hazelcast.core.IMap#putAll(Map)} call.
 */
public class HDPutAllPerMemberOperation extends MapOperation implements IdentifiedDataSerializable {

    private int[] partitions;
    private MapEntries[] mapEntries;

    @SuppressWarnings("unused")
    public HDPutAllPerMemberOperation() {
    }

    @SuppressFBWarnings("EI_EXPOSE_REP2")
    public HDPutAllPerMemberOperation(String name, int[] partitions, MapEntries[] mapEntries) {
        super(name);
    ...
    public void run() throws Exception {
        ...
        for (int i = 0; i < partitions.length; i++) {
            Operation op = new HDPutAllOperation(name, mapEntries[i]);
    ...
    @Override
    public int getId() {
        return EnterpriseMapDataSerializerHook.PUT_ALL_PER_MEMBER;
    }
}

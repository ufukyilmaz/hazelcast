 ...
 */
public final class OffHeapData extends MemoryBlock implements MutableData {
    ...
    public OffHeapData(long address, int size) {
...
//    }
    ...
    public boolean equals(Object o) {
        ...
        if (data instanceof OffHeapData) {
            return OffHeapDataUtil.equals(address(), ((OffHeapData) data).address(), bufferSize);
        }
        byte[] bytes = data.getData();
        return OffHeapDataUtil.equals(address(), bufferSize, bytes);
    }
    //CHECKSTYLE:ON
    ...
    public OffHeapData reset(long address) {
        setAddress(address);
        if (address > 0L) {

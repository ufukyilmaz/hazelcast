...
public class EnterpriseProbeTest extends AbstractDiagnosticsPluginTest {
    ...
    public void testHDStorageProbes() {
        ...
        assertContains("[name=default,unit=count,metric=map.entryCount,excludedTargets={}]=2");
        assertContains("[name=default,unit=count,metric=map.forceEvictionCount,excludedTargets={}]=");
        assertContains("[name=default,unit=count,metric=map.usedMemory,excludedTargets={}]=");

        map.remove("key2");
        plugin.run(logWriter);
        assertContains("[name=default,unit=count,metric=map.entryCount,excludedTargets={}]=1");
        ...
        map.clear();
        plugin.run(logWriter);
        assertContains("[name=default,unit=count,metric=map.entryCount,excludedTargets={}]=0");

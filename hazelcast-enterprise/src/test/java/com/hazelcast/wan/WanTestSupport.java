...
public abstract class WanTestSupport extends HazelcastTestSupport {
    private static final ILogger LOGGER = Logger.getLogger(WanTestSupport.class);
    ...
    protected WanBatchPublisherConfig targetCluster(Config config, int count) {
        return targetCluster(config, count, ConsistencyCheckStrategy.NONE);
    }

    protected WanBatchPublisherConfig targetCluster(Config config,
                                                    int count,
                                                    ConsistencyCheckStrategy consistencyCheckStrategy) {
        WanBatchPublisherConfig target = new WanBatchPublisherConfig()
        ...
        target.getSyncConfig().setConsistencyCheckStrategy(consistencyCheckStrategy);
        return target;
    }
            ...
            public void run() {
                ...
                for (HazelcastInstance instance : cluster) {
                    if (instance != null && instance.getLifecycleService().isRunning()) {
                        ...
                        DelegatingWanScheme delegate = s.getWanReplicationPublishers(wanReplicationConfigName);
        ...
        assertTrueEventually(new AssertTask() {
            ...
            public void run() {
                ...
                for (HazelcastInstance instance : targetCluster) {
                    WanEventCounters serviceCounters = getWanReplicationService(instance).getReceivedEventCounters(serviceName);

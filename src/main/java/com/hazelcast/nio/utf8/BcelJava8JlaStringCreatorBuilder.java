...
class BcelJava8JlaStringCreatorBuilder implements Constants, StringCreatorBuilder {

    @Override
    public UTFEncoderDecoder.StringCreator build() throws Exception {
        final int id = EnterpriseStringCreator.CLASS_ID_COUNTER.getAndIncrement();
        final String className = "com.hazelcast.nio.utf8.BcelStringAccessor" + id;
        ...
        Type jlaType = Type.getType(EnterpriseStringCreator.JAVA_LANG_ACCESS_CLASS);
        ...
        Constructor constructor = clazz.getConstructor(EnterpriseStringCreator.JAVA_LANG_ACCESS_CLASS);

        Field jlaField = EnterpriseStringCreator.SHARED_SECRET_CLASS.getDeclaredField("javaLangAccess");
        jlaField.setAccessible(true);

        return (UTFEncoderDecoder.StringCreator) constructor.newInstance(
                jlaField.get(EnterpriseStringCreator.SHARED_SECRET_CLASS));
    }
}

...
public class EnterpriseUTFEncoderDecoderTest {
    ...
    @Test
    public void testEnterpriseSelector() throws Exception {
        Class<UTFEncoderDecoder> clazz = UTFEncoderDecoder.class;
        Field field = clazz.getDeclaredField("INSTANCE");
        field.setAccessible(true);
        UTFEncoderDecoder instance = (UTFEncoderDecoder) field.get(clazz);
        assertTrue(instance.isHazelcastEnterpriseActive());
    }
    ...
    public void testShortSizedText_1Chunk() throws Exception {
        ...
        for (int z = 0; z < PARAMETERS.length; z++) {
            ...
            UTFEncoderDecoder.StringCreator stringCreator = EnterpriseStringCreator.findBestStringCreator(faststringEnabled,
            ...
            UTFEncoderDecoder utfEncoderDecoder = newUTFEncoderDecoder(stringCreator);
    ...
    private static UTFEncoderDecoder newUTFEncoderDecoder(UTFEncoderDecoder.StringCreator stringCreator) {
        try {
            Constructor<UTFEncoderDecoder> constructor = UTFEncoderDecoder.class
                    .getDeclaredConstructor(UTFEncoderDecoder.StringCreator.class, boolean.class);
            constructor.setAccessible(true);
            return constructor.newInstance(stringCreator, true);
        } catch (Exception e) {
    ...
    private static boolean executeTest(boolean[] parameters) {
        if (JvmUtil.getJvmVendor() == JvmUtil.Vendor.SunOracle) {
            return isOracleJava8() || !parameters[2];
        }
        if (JvmUtil.getJvmVendor() == JvmUtil.Vendor.IBM) {
            return !parameters[2];
        }
